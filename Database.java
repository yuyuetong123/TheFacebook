...
*/
    ...
    public void createNewGroup(Group newGroup){
        ...
        String[] newGroupData = {newGroup.getGroupName(), newGroup.getGroupAdmin(), String.format("\"%s\"", newGroup.getMembers())};
        String lineOfData = String.join(";", newGroupData);
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("C:\\s2137573\\FacebookDS\\groupData", true))) {
    ...
    public void updateInformationOfGroup(Group newGroup){
        
        String getGroupName = newGroup.getGroupName();
        List<String> lines = new ArrayList<>();
        
        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\s2137573\\FacebookDS\\groupData"))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] rowData = line.split(";");
                if(rowData[0].equals(getGroupName)){
                    lines.add(String.format("%s;%s;\"%s\"", newGroup.getGroupName(), newGroup.getGroupAdmin(),newGroup.getMembers()));
                }else
                    lines.add(line);
            }
        } catch (IOException e) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("C:\\s2137573\\FacebookDS\\groupData"))) {
            for (String rowData : lines) {
                String line = String.join(";", rowData);
                writer.write(line);
                writer.newLine();
    ...
    public boolean nameIsExist(String groupName){
        
        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\s2137573\\FacebookDS\\groupData"))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] rowData = line.split(";");
                if(rowData[0].equals(groupName)){
                    return true;
                }
            }
        } catch (IOException e) {
    ...
    public ArrayList<String> fingAllGroupMembersName(String groupName){
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\s2137573\\FacebookDS\\groupData"))) {
            ...
            while ((line = reader.readLine()) != null) {
                String[] rowData = line.split(";");
                if(rowData[0].equals(groupName)){
                    String[] tempMembers = rowData[2].substring(2, rowData[2].length()-2).split(", ");
                    ArrayList<String> members = new ArrayList<>();
                    for(String m : tempMembers){
                        members.add(m);
                    }
                    return members;
                }
    ...
    public String findGroupAdminName(String groupName){
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\s2137573\\FacebookDS\\groupData"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if(rowData[0].equals(groupName)){
                    ...
                    Group group=new Group(groupName,rowData[1]);
